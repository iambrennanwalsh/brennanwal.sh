  ...
  return (
    <StyledSidebar>
      ...
      <Button color="primary">
        <Image alt="freelancer" src="/images/interface/freelance.svg" />
        Hire me on freelancer.
      </Button>
      <StyledSidebarContent heading="Tags">
        Tags
        <Cloud />
      </StyledSidebarContent>
      {resources.featured &&
        resources.featured.map(article => (
          <Card
            key={article.title}
            heading={article.title}
            image={{alt: article.title, src: article.image}}
            link={{href: `/article/${article.href}`}}></Card>
        ))}
    </StyledSidebar>
  )
}
