	...
	css`
		...
		&:hover {
			box-shadow: 0 3px 0 0 ${lighten(-0.15, Theme.pallete[color!])},
				...
				0 1px 2px 1px rgba(0, 0, 0, 0) inset, 0 18px 32px -2px rgba(255, 255, 255, 0.14) inset;
		}
		&:active {
			box-shadow: 0 0px 0 0 ${lighten(-0.2, Theme.pallete[color!])}, 0 3px 0 0 rgba(0, 0, 0, 0),
...
const size = ({size}: App.ButtonProps) =>
	css`
		...
		padding: ${size == 'lg' ? '1.875rem' : size == 'md' ? '1.25rem' : '0 1.125rem'};
